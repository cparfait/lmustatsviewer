...
- ✅ **Cause réelle : Google retire les `gemini-2.5-*`.** Depuis juillet 2026, l'API renvoie `404 — This model is no longer available to new users` pour toute clé créée récemment (arrêt complet annoncé au 16/10/2026). Le coach affichait le JSON brut du fournisseur : illisible, et rien n'indiquait qu'il fallait changer de modèle.
- ✅ **`friendlyError` : cas 404 + libellés explicites** (`no longer available`, `not found for API version`, `model … not found`) → nouvelle clé `coach.errModel` (×4 langues) qui renvoie vers Configuration → Coach IA → Modèle et le lien « Voir la liste des modèles ».
- ✅ **Repli statique Google réactualisé** : `gemini-3.7-flash` / `gemini-3.6-flash` / `gemini-3.5-flash-lite`. Ce repli ne sert que sans clé ou hors ligne (avec clé, `/models` fait foi), mais il amorçait le champ sur un modèle mort.
- ✅ **Réponses Gemini vides.** Chez Gemini les tokens de raisonnement sont **décomptés de `maxOutputTokens`** : avec nos budgets courts (160 vocal, 400 analyse rapide, 600 question libre), un modèle de la génération 3 peut tout consommer en réflexion et renvoyer un texte vide — symptôme déjà entrevu (cf. commentaire de `testConnection`). `google.ts` ajoute désormais une marge de 1024 tokens au plafond. Choix d'une marge plutôt que de `thinkingConfig` : `thinkingBudget` (2.5) et `thinkingLevel` (3.x) ne s'appliquent pas aux mêmes générations, un champ inadapté fait échouer la requête en 400 — la marge, elle, est valable partout.
- ✅ **Le champ Modèle ne dit pas qu'il est libre.** Deuxième moitié du retour : la valeur enregistrée (`gemini-2.5-flash`, choisie il y a des mois) reste affichée telle quelle, sans placeholder visible ni indication qu'on peut la remplacer. `AiModelPicker` affiche maintenant, sous le champ, une note permanente `config.aiModelHint` — « Champ libre — exemple : `gemini-3.7-flash` » — dont l'exemple est tiré de `fallbackModels[0]` du fournisseur : jamais périmé tant que le repli est tenu à jour, et absent chez Ollama dont les tags sont locaux (repli `aiModelHintPlain`, idem pour le placeholder).
- ✅ **Champ Modèle : liste d'abord, saisie en secours** (design final après deux allers-retours dans la session : « brouillon » → champ libre seul → retour de la liste demandé, capture harness à l'appui). Ce qui a changé entre-temps et rend la liste pertinente : avec les clés par fournisseur, le sondage `/models` est fiable — le menu montre la **liste vivante** du fournisseur, pas un repli figé. « Saisir manuellement… » reste en dernière option (modèle trop récent) ; saisie seule quand le sondage est vide (hors ligne, custom muet). Un id hors liste est affiché tel quel, jamais écrasé (règle du 08/08).
- ✅ **Changement de fournisseur → modèle remis à zéro** (`setAIProvider`) puis ré-amorcé par le sondage. Découvert au test Ollama du mainteneur : le champ gardait `gemini-2.5-flash` avec Ollama actif — combinaison qui ne pouvait qu'échouer.
- ✅ **Bouton « Tester l'IA »** (demande du mainteneur : « quelque chose de sympa ») à côté du test de connexion : envoie au modèle les stats réelles (nom du pilote, dernière course via `get_sessions_list` filtré `Race` trié `Date` — ⚠️ pas `timestamp`, colonnes V1 —, voiture/circuit favoris du Dashboard, carrière) et affiche un message radio 2-3 phrases dans la langue de l'UI. La réponse est affichée ET lue par `announce()` (même chaîne radio/TTS que les annonces en course, priorité `normal`). Valide la chaîne complète clé→modèle→données→langue→TTS, là où « Tester » ne valide que l'appel. Sans données en base : message dédié `aiTestFunNoData` (×4).
- ✅ **Circuits US Track Pass intégrés au coaching** (demande mainteneur « 4 circuits américains »). Réalité vérifiée (recherche web) : le Pack 1 (28/07, V1.4) n'apporte QUE **Daytona + Laguna Seca** ; packs 2/3 (sept./déc., Watkins Glen + Indianapolis pressentis) pas encore sortis — COTA/Sebring/Road Atlanta étaient déjà couverts.
  - **Guides vidéo** ajoutés (4 combos, vérifiés via oEmbed YouTube) : Daytona Hypercar (GO Setups), Daytona LMGT3 + Laguna Hypercar + Laguna LMGT3 (HYMO Academy). Unleashed Drivers n'a pas couvert ces circuits → nouveau champ `channel?` sur `VideoGuide` (absent = Unleashed, entrées historiques) et source dynamique dans `buildVideoGuideText`. Crédits mis à jour : page Crédits (2 entrées) + `coach.sources` ×4.
  - **Freinages : référence approximative auto-rédigée, sourcée honnêtement** (le mainteneur voulait une référence ; ApexPoints — app JS non scrapable — ne couvre pas ces circuits, transcriptions YouTube vides — hotlaps muets « Heat. Heat. » —, pas d'article Coach Dave gratuit). Nouveau champ `BrakingTrack.source?` (absent = ApexPoints, entrées historiques) affiché dans l'en-tête du contexte : les 2 circuits portent `community track notes (approximate — pending ApexPoints coverage)`. Daytona 5 virages (T1, International Horseshoe, Kink, West Horseshoe, chicane Le Mans — numérotation vérifiée : IMSA/Studio-397/Autometrics) ; Laguna 7 virages (Andretti, T3, T5, T6, Corkscrew, Rainey, T11) ; ×3 classes, vitesses en «≈», la TECHNIQUE est la partie fiable. **À remplacer par ApexPoints dès disponible.** Smoke-test tsx : matching par nom DB réel OK (freinage + vidéo).
  - Pas de transcriptions ASR pour ces vidéos (tenté via youtube-transcript-api : sous-titres vides, vidéos sans commentaire).
- ✅ **TTS : temps au tour, 2e retour** : « 2:26.707 » → « 2 26 707 » était lu « vingt-six mille sept cent sept » (séparateur de milliers). Virgules entre les blocs : « 2, 26, 707 ».
- ✅ **Lecture d'analyse : pause après les titres.** `stripMarkdown` retirait `###`/`**gras**` sans ponctuation → « Point fort principal » s'enchaînait avec le paragraphe. Les lignes-titres se terminent désormais par un point (`punctuateTitle`), ce qui fait marquer la pause au TTS.
- ✅ **TTS : normalisation phonétique du jargon course** (déclencheur : « P13 » prononcé « pétré »). `normalizeForTts(text, lang)` dans `speak()` ET `announce()` (chemins distincts) : « P13 »/« T4 » → espacés ; temps `m:ss.mmm` → convention radio « une 42 881 » (retour mainteneur : pas de « minute »/« virgule » ; « une » au féminin en FR, chiffre brut ailleurs ; minute à 1 chiffre pour ne pas toucher une heure « 14:30 ») ; écarts `+0.5s`/`-0.3s` → « plus/moins … secondes » ; `km/h`, `°C` → en toutes lettres ; catégories (GT3, LMP2, LMGT3, LMDh, GTE — liste fermée, pas de motif générique) → épelées ; décimales `x.y` → mot local hors EN (lookahead pour les suites « 1.0.3 »). Mots de liaison par langue (`TTS_WORDS` ×4). Motifs validés par dry-run Node (6 phrases). Le texte AFFICHÉ n'est jamais modifié.
- ✅ **Amorçage : saut des modèles retirés.** Google liste encore les `gemini-2.5-*` pour les clés anciennes → l'amorçage prenait `models[0]` = un modèle condamné, alerte d'entrée. `refreshAiModels` amorce sur le premier modèle NON `isRetiredModel`. Ligne « Fournisseur » renommée **« Fournisseur actif »** (le titre de section « Fournisseurs » créait un doublon) + tooltip actualisé ×4. Lien « Voir la liste des modèles » limité au mode saisie (doublon du menu en mode liste).
- ✅ **Avertissement « modèle retiré » + remplacement en un clic** (« Utiliser `gemini-3.7-flash` »). Pas de réécriture automatique de la config : c'est exactement la régression corrigée le 08/08 (`refreshAiModels` écrasait un id saisi à la main à chaque ouverture de la page). Nouveau hook optionnel `AIProvider.isRetiredModel?(id)` : implémenté pour Google seulement (`/^(models\/)?gemini-[012](\D|$)/`), il déclenche une ligne ambre sous le champ avant même le premier appel. Choix d'un motif de génération plutôt que d'une liste d'ids : un motif incomplet ne produit **aucun faux positif** (au pire, pas d'avertissement) — c'est ce qui le distingue de la table de coûts supprimée le 08/08, qui affichait des valeurs fausses en silence.
- ✅ **Même bug chez OpenAI, corrigé préventivement** (inspection du modèle de config de `deepseek-harness`, demandée par le mainteneur : leurs profils `compat` pointent la divergence `maxTokensField`). Les modèles de raisonnement OpenAI (o-séries, GPT-5.x) **rejettent `max_tokens`** en 400 `unsupported_parameter` — or `openai-compat.ts` l'envoyait en dur, et notre repli suggérait `o4-mini`. Nouveau champ `maxTokensField` dans `OpenAICompatConfig` : `max_completion_tokens` pour OpenAI (accepté par tous leurs modèles actuels), `max_tokens` conservé pour DeepSeek/Mistral/OpenRouter/passerelles. Repli OpenAI rafraîchi : `gpt-5-mini` / `gpt-5.2` / `gpt-5.5` (ids vérifiés dans la doc officielle ; la famille « GPT-5.6 Luna/Terra/Sol » n'a pas d'id documenté → pas mise).
- ✅ **Marge « thinking » généralisée aux 3 chemins de sortie** (`google.ts` +1024, `openai-compat.ts` +1024, `ollama.ts` `num_predict`+1024) — le custom l'a mise en évidence : **testé en réel** contre l'Ollama local du poste (API OpenAI-compat ET native), `gemma4:12b` avec `max_tokens:16` renvoie `content:""` + tout le budget en `reasoning`/`thinking` (`finish_reason: length`) ; avec la marge, `"OK"` + `finish_reason: stop`. Anthropic non concerné (le thinking y est opt-in et nous ne l'activons pas).
- ✅ **Refonte complète en « panneau fournisseurs » façon harness** (2e itération dans la même session, sur demande « je veux vraiment la même chose ») : TOUS les fournisseurs — intégrés comme customs — deviennent des **cartes** avec **clé chiffrée propre** + pastille d'état, deux boutons d'ajout (« Ajouter un fournisseur » = catalogue des 7 intégrés ; « … personnalisé » = nom + URL). Conséquences structurantes :
  - **Fin des créneaux de clé globaux** `aiApiKey`/`aiVoiceApiKey` → `aiProviderKeys[id]` pour tous (générique Rust `ai_key_enc_<id>`). **Migration au 1er chargement** : clé analyse → fournisseur actif, clé vocale → fournisseur vocal, liste `ai_provider_list` amorcée avec ces fournisseurs. Les anciens créneaux ne sont plus lus mais **pas effacés** (retour arrière de version possible).
  - Les menus (analyse + vocal) ne proposent QUE les fournisseurs configurés ; l'ancien champ « Clé API vocale » disparaît (une clé par fournisseur, plus de doublon analyse/vocal). `resolveVoiceTarget`, `AICoachPanel`, `coachPhrasebank`, test/probe ConfigV2 : tous sur `aiProviderKeys`.
  - Non repris de leur formulaire : « Customized settings » (URL override d'un intégré, liste de modèles par carte, protocole) — un seul protocole chez nous (`openai-completions`) et le modèle se choisit dans le champ Modèle.
- ✅ *(étape intermédiaire, remplacée le jour même)* **Gestionnaire de fournisseurs custom façon deepseek-harness** (demande explicite du mainteneur après capture de leur panneau Settings → Models ; a remplacé dans la même session une v1 « URL unique » jamais publiée). Fournisseurs OpenAI-compatibles **multiples et nommés**, gérés en cartes (ajouter/modifier/supprimer) avec **pastille d'état** (sondage `GET /models` au montage, pas de polling).
  - **Backend** : 2 commandes génériques `ai_set/get_provider_key(provider_id)` → clé chiffrée AES-GCM par fournisseur sous `ai_key_enc_<id>`, id validé (`[A-Za-z0-9_-]{1,64}`). Les intégrés gardent le créneau global `aiApiKey` (comportement inchangé).
  - **Frontend** : définitions non secrètes en config JSON (`ai_custom_providers`), ids `custom-<8 hex>`, registre dynamique `allProviders()` = `PROVIDERS` + customs (injection `setCustomProviders()` par le store, pas d'import du store dans les providers — cycle). `AIProvider.custom: true` → clé résolue par fournisseur dans **tous** les chemins : ConfigV2 (test + sondage modèles), `AICoachPanel`, `resolveVoiceTarget` (vocal), `coachPhrasebank`. Champ Clé API global masqué pour un custom (sa clé vit sur sa carte). Suppression d'un custom sélectionné → retour à `google` (+ purge du vocal s'il l'utilisait).
  - `needsKey: false` (LM Studio/vLLM sans clé) mais Authorization envoyée si clé présente. Ajout d'un fournisseur → sélection immédiate.
  - Non repris de deepseek-harness (hors périmètre pour une app desktop grand public) : `settings.yaml`, architecture plugins, modalités par modèle, profils compat par route.
  - ⚠️ Découverte au passage : `npx tsc --noEmit -p tsconfig.json` **ne vérifie rien** (tsconfig « solution » à references, sans fichiers) — le vrai check est `tsc -b`, comme dans `npm run build`. Les « tsc OK » précédents de la session ne prouvaient rien ; repassé en `tsc -b --force` : OK.
- 📋 **Prochaine étape** : réponse à l'utilisateur (taper l'id du modèle dans le champ, ex. `gemini-3.6-flash`), puis publier une **1.0.3** — l'entrée changelog est déjà en place en `dev: true`, `version.json` reste à bumper. Tester en vrai le fournisseur custom contre un endpoint local (LM Studio/Ollama en mode OpenAI) avant la release.
...
- ✅ **Workflow *Release* vert en 24 min** (run `31542686306`). `releases/tags/v1.0.2` renvoie 404 en anonyme et `releases/latest` reste sur v1.0.1 : le brouillon est bien créé et invisible des clients, comme attendu.
  - ⏱️ **`RELEASE.md` corrigé** : l'estimation « ~5–10 min » était fausse. Un bump modifie `Cargo.toml` → une partie du cache Cargo est invalidée → `duckdb` (feature `bundled`) recompile tout son C++ sur le runner. Une release tombe donc quasi systématiquement dans le cas « cache froid ». Compter ~25 min.
- ✅ **v1.0.2 PUBLIÉE** (2026-08-12, 10:14 UTC). Brouillon vérifié avant publication : `latest.json` (1,74 KB), installeur (43,7 MB) et `.exe.sig` (428 o) présents — jeu d'assets identique à la 1.0.1, et **taille d'installeur inchangée** (+0,02 MB), donc les voix Piper / modèles Vosk ne se sont pas retrouvés bundlés par accident. `releases/latest` redirige bien sur `v1.0.2` : l'auto-update est actif pour tout le parc.
- 📋 **Prochaine étape** : attendre la confirmation en jeu de **fugltind** et **Pillot69**, qui ont remonté les deux problèmes — c'est le seul moyen de valider réellement les correctifs (fenêtre console et niveau audio ne se constatent qu'avec le jeu lancé en plein écran).
...
- ✅ **Publication en 1.0.0 — auto-update fonctionnel.** La **dernière version réellement en production est la 0.9.4** (confirmé par le mainteneur) : `latest.json` à `1.0.0` est donc strictement supérieur au parc installé, la notification de mise à jour partira normalement.
  - ⚠️ **Piège à connaître pour la suite** : les tags `v1.0.1` → `v1.0.7` existent sur le remote mais **ne correspondent à aucune release publiée**. Tous datent des 28-29 mai, `v1.0.5` et `v1.0.7` pointent sur le même commit (`baee068`), et les commits de cette journée sont une série de correctifs du pipeline (`fix: artefacts Tauri v2`, `fix: URL latest.json`, `fix: TAURI_SIGNING_PRIVATE_KEY_PASSWORD`) — ce sont des tentatives de mise au point.
  - 🔑 **Mécanique à retenir** : le workflow ne passe le tag qu'en `tagName`/`releaseName`, **jamais dans la version**. Celle-ci vient uniquement de `version.json` via `build.rs` → `tauri.conf.json` → `latest.json`. Un tag `vX.Y.Z` sans bump de `version.json` produit donc un installeur qui se déclare à l'ancienne version — c'est exactement ce qui s'est passé pour les sept tags de mai (`git show v1.0.7:version.json` renvoie `1.0.0`). `release.ps1` fait le bump + commit correctement (l. 127-155) ; il n'avait pas été utilisé.
...
- ✅ **v1.0.0 PUBLIÉE** (tag `v1.0.0` → `4df3baf`). Chaîne de mise à jour vérifiée : `releases/latest/download/latest.json` renvoie HTTP 200, `version: 1.0.0`, signature présente, URL pointant sur `LMU.Stats.Viewer_1.0.0_x64-setup.exe` (les espaces convertis en points par GitHub sont bien pris en compte). Cibles `windows-x86_64` et `windows-x86_64-nsis`.
- ❌ **Deux échecs CI avant d'y arriver** — les deux dus à des angles morts, pas à des régressions :
  1. **DuckDB ne compilait pas** (`'stdext' is not a class or namespace name`, `/std:c++17` et `/EHsc` absents). `duckdb` est en feature `bundled` → tout le C++ est recompilé sur le runner. Il a été ajouté le 10 juin (`ac553a3`) et **aucun build CI n'avait tourné depuis** : cette chaîne n'avait jamais été exercée. Le runner `windows-latest` embarque MSVC 14.51, le poste de dev 14.44. Corrigé par `4df3baf` (duckdb 1.10505.0 + runner épinglé `windows-2022` + `CXXFLAGS`).
  2. **Le secret `TAURI_SIGNING_PRIVATE_KEY` n'existait tout simplement pas** sur le dépôt. `${{ secrets.… }}` se résolvait en chaîne vide → tauri échouait sur `Missing secret comment in secret key`, message trompeur qui pointe vers un problème de format. C'est très probablement ce qui bloquait déjà la session de mise au point du 29 mai, et pourquoi aucune release n'avait jamais abouti.
- ✅ **Réorganisation des branches** : `main` (app V1 PHP) archivé en `v1-legacy`, `V2` renommé `main` et remis en branche par défaut. Le POC de migration du 12 mai (branche `v2` minuscule, 2 commits non contenus ailleurs) est préservé par le tag `poc-migration-20260512`. `release.ps1` pousse désormais sur `main` (`10a43cb`).
- 📋 **Prochaine étape** : pour la prochaine version, **toujours bumper `version.json` avant de taguer** — de préférence via `release.ps1`, qui enchaîne bump, commit, tag et push. Penser aussi à rafraîchir l'entrée 1.0.0 de `src/lib/changelog.ts`, datée `2026-06` et antérieure aux travaux de juillet-août (coach par virage, voix téléchargeables, visite guidée, numéro de voiture).
