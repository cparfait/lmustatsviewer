...
- 📋 **Prochaine étape** : vérifier le brouillon (présence de l'`.exe` **et** de `latest.json` — sans ce dernier l'auto-update ne voit rien), coller les notes de version, puis **Publish release**. Attendre ensuite la confirmation de l'utilisateur qui a signalé les saccades : les deux correctifs ne sont vérifiables qu'en jeu, en plein écran.
