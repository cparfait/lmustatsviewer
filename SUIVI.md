...
- 📋 **Prochaine étape** : publier le brouillon `v1.0.0` (Actions → vérifier le `.exe` **et** `latest.json`). Pour les versions suivantes, **toujours bumper `version.json` avant de taguer** — de préférence via `release.ps1`, qui enchaîne bump, commit, tag et push.
